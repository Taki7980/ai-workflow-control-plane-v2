...
def build_parser():
    ...
    q=sp.add_parser("setup",help="connect AI Workflow to the current project; safe to rerun"); q.add_argument("--project-name"); q.add_argument("--json",action="store_true",help="print machine-readable setup result"); q.add_argument("--create",action="store_true",help="explicitly create the project root when it does not exist"); idx=q.add_mutually_exclusive_group(); idx.add_argument("--no-index",action="store_true",help="skip index construction during setup"); idx.add_argument("--full-index",action="store_true",help="force a full index rebuild instead of auto/incremental setup"); q.set_defaults(func=cmd_setup)
