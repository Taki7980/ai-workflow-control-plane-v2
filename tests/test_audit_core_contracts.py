...
class SetupSafetyTests(unittest.TestCase):
    ...
    def test_setup_reuses_incremental_index_state(self):
        from ai_workflow.bootstrap import setup
        with tempfile.TemporaryDirectory() as td:
            root = Path(td); (root / "sample.py").write_text("def alpha():\n    return 1\n", encoding="utf-8")
...
class CoreContractTests(unittest.TestCase):
    ...
    def test_token_estimator_is_pluggable(self):
        ...
        class Fixed:
            def estimate(self, text: str) -> int: return 7
    ...
    def test_selector_enforces_candidate_complexity_bound(self):
        ...
        selected, diagnostics = select_context("query", items, budget_chars=10000, max_selector_candidates=3)
    ...
    def test_atomic_write_preserves_old_file_when_replace_fails(self):
        ...
        with tempfile.TemporaryDirectory() as td:
            target = Path(td) / "state.txt"; target.write_text("old", encoding="utf-8")
            with patch("ai_workflow.io_utils.os.replace", side_effect=OSError("boom")):
                with self.assertRaises(OSError): atomic_write_text(target, "new")
    ...
    def test_cli_exposes_setup_safety_and_hash_verification_flags(self):
        ...
        setup_args = build_parser().parse_args(["--root", "x", "setup", "--create", "--no-index"])
        self.assertTrue(setup_args.create); self.assertTrue(setup_args.no_index)
        index_args = build_parser().parse_args(["--root", "x", "index", "--incremental", "--verify-hashes"])
